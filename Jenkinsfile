    ...
    try {
        ...
        stage('run tests') {
            parallel('run libc-tests': {
                run_test('run-libc-tests', 't2.micro', 1, 'none', [
                    ['RUNFILE', '/opt/libc-tests/runfiles/default.run']
                ])
            }, 'run os-tests': {
                run_test('run-os-tests', 't2.micro', 1, 'none', [
                    ['RUNFILE', '/opt/os-tests/runfiles/default.run']
                ])
            }, 'run util-tests': {
                run_test('run-util-tests', 't2.micro', 1, 'none', [
                    ['RUNFILE', '/opt/util-tests/runfiles/default.run']
                ])
            }, 'run zfs-tests': {
                run_test('run-zfs-tests', 'm4.large', 6, 'run-zfs-tests', [
                    ['RUNFILE', '/opt/zfs-tests/runfiles/delphix.run']
                ])
            }, 'run zloop': {
                run_test('run-zloop', 'm4.large', 6, 'none', [
