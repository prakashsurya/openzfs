...
again:	if ((t = utq->utq_freelist) != NULL &&
		...
		mutex_exit(&utq->utq_lock);

		t = umem_alloc(sizeof (utaskq_ent_t), utqflags);

		mutex_enter(&utq->utq_lock);
		if (t != NULL)
			utq->utq_nalloc++;
	}
	return (t);
    ...
    int maxalloc, uint_t flags)
{
	...
	VERIFY0(rwlock_init(
	    &utq->utq_threadlock, LOCK_NORMAL | LOCK_ERRORCHECK, NULL));
	VERIFY0(mutex_init(
	    &utq->utq_lock, LOCK_NORMAL | LOCK_ERRORCHECK, NULL));
	VERIFY0(cond_init(
	    &utq->utq_dispatch_cv, LOCK_NORMAL | LOCK_ERRORCHECK, NULL));
	VERIFY0(cond_init(
	    &utq->utq_wait_cv, LOCK_NORMAL | LOCK_ERRORCHECK, NULL));
	VERIFY0(cond_init(
	    &utq->utq_maxalloc_cv, LOCK_NORMAL | LOCK_ERRORCHECK, NULL));
